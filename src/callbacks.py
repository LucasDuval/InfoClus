...
def register_callbacks(app):
    ...
    def update_maindiv(dataset_name, change_hyper, embedding_name, alpha, beta, runtime_id, minAtt):
        ...
        if trigger_id == 'dataset-select':
            ...
            infoclus_object_path = os.path.join(PROJECT_ROOT, 'data', dataset_name, f'{dataset_name}_{embedding_name}.pkl')
    ...
    def select_cluster_explanation(cluster_id, dataset_name, emb_name):
        file_path = os.path.join(PROJECT_ROOT, 'data', dataset_name, f'{dataset_name}_{emb_name}.pkl')
    ...
    def select_embedding(embedding_name_show, dataset_name, emb_name_comp):
        ...
        file_path = os.path.join(PROJECT_ROOT, 'data', dataset_name, f'{dataset_name}_{emb_name_comp}.pkl')
        if os.path.exists(file_path):
            with open(file_path, 'rb') as f:
                infoclus = pickle.load(f)
        else:
